...
class BookRestControllerTest {
	...
	@Test
	void getAlltheBooks() throws Exception {

			Logger log = Logger.getLogger(AuthenticationRestController.class);
			String token = UserConnection();
			mockMvc.perform(get("/books").contentType("application/json").header("Authorization", "Bearer "+token)).andExpect(status().isOk()).andDo(print());
	}
